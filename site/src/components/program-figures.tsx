 ...
 *
 * FY24 actuals: State A (cited via fy2024_fact_id) OR State C if fact_id is null.
 * FY25 total + FY26 total: from trajectory in THOUSANDS → State C (uncited per scope decision 1).
 * FY25→26 change: derived/uncited → State C.
 *
 * Per plan: "trajectory figures in THOUSANDS — state C (uncited — pass no factId/xmlPath)"
 */
...
export function ProgramFigures({ program }: ProgramFiguresProps) {
  const { fy2024_actual_millions, fy2024_fact_id, trajectory } = program;

  // FY25 and FY26 totals come from trajectory (USD thousands) — state C
    ...
    <section aria-labelledby="figures-heading" className="mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="text-xl font-bold">
            {fy25 !== null ? (
              <Cite value={fy25} units="USD thousands" />
            ) : (
              <span className="text-muted-foreground text-sm">—</span>
            )}
          </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="text-xl font-bold">
            {fy26 !== null ? (
              <Cite value={fy26} units="USD thousands" />
            ) : (
              <span className="text-muted-foreground text-sm">—</span>
            )}
          </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="text-xl font-bold">
            {fy2526Change !== null ? (
              ...
              >
                <Cite value={fy2526Change} units="USD thousands" />
                {fy2526Pct !== null && (
                  <span
      ...
      {trajectory && (
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <TrajectorySpark trajectory={trajectory} />
        </div>
      )}
    </section>
  );
